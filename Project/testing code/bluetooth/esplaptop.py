# espLaptop.py (ESP32 plugged into laptop)
import bluetooth
import time
import sys
...
# ---------- TARGET (CAR) ----------
# Put YOUR car ESP32 MAC here (same as your old working code)
...
def send_text(text):
    if not ready_to_write:
        return
    try:
        # write with response (1) is ok
        ble.gattc_write(conn_handle, char_handle, text.encode(), 1)
        print("sent:", text)
...
def bt_irq(event, data):
    ...
    if event == _IRQ_SCAN_RESULT:
        ...
        ready_to_write = True
        print("READY.")
        print("Type a number 0..1023 and press Enter.")
        print("Example: 250  400  800  0(stop)")
    ...
    if ready_to_write:
        ...
        if line:
            ...
            try:
                ...
                print("Type only a number 0..1023")
                time.sleep_ms(10)
                continue
